...
function ProjectCard({ project, onClick }) {
  return (
    ...
    >
      {/* colour stripe */}
      <Box h="3px" bg={STATUS_STRIPE[project.project_status] ?? '#9ca3af'} />
        ...
        >
          <VStack gap={1.5} align="stretch">
            <Flex align="center" gap={2}>
              ...
              <Text fontSize="12.5px" color="gray.600" noOfLines={1}>
                {project.client}
              </Text>
            </Flex>
            <Flex align="center" gap={2}>
  ...
  return (
    ...
    return (
        <Box
          ...
          h="26px"
          borderRadius="md"
          bg={view === key ? 'white' : 'transparent'}
          color={view === key ? 'gray.700' : 'gray.400'}
          boxShadow={view === key ? '0 1px 3px rgba(0,0,0,0.10)' : 'none'}
          transition="all 0.12s"
          cursor="pointer"
          border="none"
          _hover={{ color: view === key ? 'gray.700' : 'gray.600' }}
        >
          <Icon size={14} strokeWidth={view === key ? 2.4 : 1.8} />
        </Box>
    )})}
    </Flex>
  )
